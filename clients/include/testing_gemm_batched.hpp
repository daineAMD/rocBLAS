...
void testing_gemm_batched(const Arguments& arg)
{
    ...
    hipMalloc(&dB, batch_count * sizeof(T*));
    hipMalloc(&dC, batch_count * sizeof(T*));
    ...
    for(int i = 0; i < batch_count; i++)
    {
        ...
        hipMalloc(&B_1[i], size_b * sizeof(T));
        hipMalloc(&C_1[i], size_c * sizeof(T));
    }

    int last = batch_count - 1;
    if((!A_1[last] && size_a) || (!B_1[last] && size_b) || (!C_1[last] && size_c))
    {
        CHECK_HIP_ERROR(hipErrorOutOfMemory);
        return;
    }
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        for(int i = 0; i < batch_count; i++)
        {
            CHECK_HIP_ERROR(hipMemcpy(hC_1[i], C_1[i], sizeof(T) * size_c, hipMemcpyDeviceToHost));
        }
